...
REM ============================================================================
REM PayloadPack - Rust + Debug Build Script with Telegram Progress
REM ============================================================================

echo.
echo ========================================
echo  PayloadPack Debug Build (with Rust)
echo ========================================
echo.

set PROJECT_ROOT=%~dp0
set RUST_PROJECT=%PROJECT_ROOT%app\src\main\rust
...
call :SendTelegramStart "PayloadPack Debug Build Started" "Initializing..." "0"
...
cd /d "!RUST_PROJECT!"
cargo ndk -t arm64-v8a -o "!JNILIBS_DIR!" build
...
if exist "%JNILIBS_DIR%\arm64-v8a\libpayloadpack.so" (
    echo OK arm64-v8a/libpayloadpack.so
) else (
    echo ERROR: Native library not found! >> "%LOG_FILE%"
    echo Looking for library in: %JNILIBS_DIR%\arm64-v8a\
    dir "%JNILIBS_DIR%\arm64-v8a\" 2>nul
    call :BuildFailed "Native library libpayloadpack.so not found!"
    pause
    exit /b 1
)
echo.

call :UpdateProgress "Verify Library" "Native library verified!" "libpayloadpack.so OK" "35"
...
if exist "%APK_PATH%" (
    if %SKIP_INSTALL%==0 (
        ...
        if !ERRORLEVEL! equ 0 (
            ...
            echo WARN: Instalasi gagal, mungkin perlu uninstall manual dulu.
            call :UpdateProgress "Installing APK" "Installation warning" "May need manual uninstall" "80"
        )
    ) else (
...
if %UPLOAD_TO_TELEGRAM%==1 (
    ...
    call :UpdateProgress "Upload to Telegram" "Renaming APK..." "PayloadPack-version-debug.apk" "85"
...
if exist "%PROJECT_ROOT%telegram_helper.ps1" (
